 ...
 * Could either be a right flipper or a left flipper.
 */
public class LeftFlipper implements Gadget{
    
    //Keeping a copy of the original x and y before adding the board offset (1). 
     ...
     * @param rightflipper boolean true if this flipper is a right-flipper, false if this flipper is
     * a left-flipper.
    ...
    private void checkRep() throws RuntimeException{
        if (this.x<0 || this.x>19 || this.y<0 || this.y>19){
            throw new RuntimeException("LeftFlipper's position cannot be outside of the board's bounds!!!");
        }
        if (!(this.orientation ==0 || this.orientation ==90 || this.orientation ==180 ||this.orientation ==270)){
            throw new RuntimeException("LeftFlipper's orientation should be 0|90|180|270!!!");
        }
    }
     ...
     * @return boolean true if this flipper is in a vertical position, otherwise false. 
     */
    ...
    /**
     * A flipper generates a trigger whenever a ball hits it.
     * When a flipper’s action is triggered, the flipper rotates at a constant 
     * angular velocity of 1080 degrees per second to a position 90 degrees away 
     * from its starting position. When its action is triggered a second time, the 
