...
/**
 * ##############Server/Client Overview and Concurrency Arguments for Pingball
 * Game###############
 * 
 * Overview: The Server has n+1 threads, given n clients. The first thread
 * continuously listens for the user to input join commands. The last n threads
 * communicate with the client through the socket.
 * 
 * Each client has 4 threads running. (1) The first thread GamePlayer plays the
 * game. (2) The main thread inside PingballClient displays the game. (3) The
 * third thread Communicator communicates with the server through the socket.
 * (4) The fourth thread inside Communicator continuously checks the state of
 * the board at regular intervals to see if a ball collided with a wall.
 * 
 * Strategy: When a client first connects to the server, it sends to the server
 * the name of its board. The server stores the name and other info inside the
 * hashmap which is explained below in the comment. When a user types in a join
 * command, the thread inside the server listens and updates the hashmap for
 * both boards mentioned in the command.
 * 
 * When (4) determines that a ball did collide with a wall, the thread sends to
 * the server the name of the ball, its location, and its velocity to the
 * server. The server looks up the wall in the hashmap to determine if it's
 * invisible. If the ball hit a visible wall, the server sends message back to
 * the client saying play the game as usual. If the ball hit an invisible wall,
 * the server sends a message back saying the client should delete the ball. The
 * server also sends a message to the client's neighbor to create a new ball at
 * an appropriate location.
 * 
 * Concurrency Argument: The strategy uses two shared memory structures. On the
 * server side, the n threads access the hash map, and the last thread modify
 * the hash map. We are using ConcurrentHashMap which provides thread-safe
 * operations. Since the dictionary key comprises unique board names, when a
 * single ball hits a wall, each client thread will be accessing different keys,
 * so the methods do not block. When two balls hit a wall from a single client,
 * the server calls a get method on the hashmap on the same key which is a
 * thread safe method for ConcurrentHashMap according to Oracle.com.
 * 
 * On the client side, we have four threads running. (1) calls moveAllballs
 * which accesses concurrentHashMap balls (2) calls display() which accesses 2d
 * String array and ConcurrentHashMap of positionofGadgets (3) calls either
 * insertBall or deleteBall which also accesses balls (4) calls whichWallGotHit
 * which accesses List of Walls and balls
 * 
 * (2) does not block because none of the other threads need to access 2d String
 * array and positionofGadgets (1),(3), and (4) access balls by calling a get
 * method. As mentioned above, ConcurrentHashmap provides thread-safe get
 * method. Therefore, there is no risk of interleaving here. (4) accesses List
 * of Walls which is not accessed by other threads after the board is
 * initialized. We can apply the same argument for the ConcurrentHashMap of
 * positionofGadgets as well. PositionofGadgets does not mutate after
 * initialization. There is no deadlock because none of the threads are waiting
 * for each other to finish access to any of the data strcutures mentioned
 * above.
 * 
 * Therefore, the access to the shared memory within Server and Client and
 * between them is thread-safe.
 * 
 * @author jonathan
 * 
 */
public class PingballServer {
    ...
    // ConcurrentHashMap<String, Tuple> created when joining the boards is as
    // follows:
    //
    // top bot left right top bot left right
    // { "board1" : ([neighbor1, n2, n3, n4] , [true, true, true, true],
    // board1's socket, [true, false, false, true])
    // "neighbor1": ([null, client1, null, null],[false, true, false, false],
    // neighbor1's socket, [false, true, true, false])
    // "board3" : ([n1, null, null, n4] , [true, false, true, false], board3's
    // socket, ...)
    // ... }
    // Thus, each board name has a list of all the neighboring board names which
    // are initialized as null.
    // Each board name also has a list of booleans indicating if the top,
    // bottom, left, or right wall is
    // invisible or not. Each board name has a socket that corresponds to that
    // particular client thread.
    // The final list is a list of booleans that indicate if each neighbor has
    // agreed to chat with that client
    // Quadruple has a list of strings and a list of booleans.
    private ConcurrentHashMap<String, Quadruple<List<String>, List<Boolean>, Socket, List<Boolean>>> neighbors = new ConcurrentHashMap<String, Quadruple<List<String>, List<Boolean>, Socket, List<Boolean>>>();
    ...
    /**
     * Method used for telling clients that their wall has been joined Gets all
     * valid neighbors, get their sockets, and send messages.
     * 
     * @param name
     * @throws IOException
     */
    ...
    /**
     * get the socket corresponding to chatName and send the message to that
     * board
     * 
     * @param chatName
     * @param msg
    ...
    /**
     * join boards horizontally. Left board's right wall is joined with Right
     * board's left wall
     * 
     * @param left
     *            board
     ...
     * @throws IOException
     */
    private void makeHorizNeighbors(String[] words)
            throws IllegalArgumentException, IOException {
        if (!words[1].contains("_left") || !words[2].contains("_right")) {
            throw new IllegalArgumentException(
                    "Enter: h board1_left board2_right");
        }
        ...
        // if left is in the dictionary, and the existing right neighbor does
        // not
        // match the current bottom neighbor
        String prevRight = neighbors.get(left).getOne().get(3);
        if (neighbors.get(left).getTwo().get(3) && !prevRight.equals(right)) {
            setNullifyBoards(prevRight, 2);
        }

        // if right is in the dictionary, and the existing left neighbor does
        // not
        // match the current left neighbor
        String prevLeft = neighbors.get(right).getOne().get(2);
        if (neighbors.get(right).getTwo().get(2) && !prevLeft.equals(right)) {
    ...
    /**
     * join boards vertically Top board's bottom wall is joined with Bottom
     * board's top wall
     * 
     * @param top
     * @param bottom
     * @throws IOException
     */
    private void makeVerticNeighbors(String[] words)
            throws IllegalArgumentException, IOException {
        if (!words[1].contains("_top") || !words[2].contains("_bottom")) {
            throw new IllegalArgumentException(
                    "Enter: v board1_top board2_bottom");
        }
        ...
        // if top is in the dictionary, and the existing bottom neighbor does
        // not
        // match the current bottom neighbor
        String prevBottom = neighbors.get(top).getOne().get(1);
        if (neighbors.get(top).getTwo().get(1) && !prevBottom.equals(bottom)) {

            setNullifyBoards(prevBottom, 0);
        }

        // if bottom is in the dictionary, and the existing top neighbor does
        // not
        // match the current top neighbor
        String prevTop = neighbors.get(bottom).getOne().get(0);
        if (neighbors.get(bottom).getTwo().get(0) && !prevTop.equals(top)) {
     ...
     * 
     * @param s1
     *            the board that needs to be modified.
     * @param s2
     *            the neighboring board
     * @param i2
     *            indicates which wall needs to be marked as invisible
     * @throws IOException
     */
    ...
    /**
     * the main joinBoards function parses the join command
     * 
     * @param command
     * @throws IOException
     * @throws IllegalArgumentException
     */
    public void joinBoards(String command) throws IllegalArgumentException,
            IOException {
    ...
    /**
     * @param name
     *            : board that wants to chat
     * @param nameWantsChat
     *            : if that board wants to chat
     * @return if both name and neighbor want to chat
     */
    public boolean checkChatAgreement(String name, boolean nameWantsChat,
            String neighbor) {
        int nameIndex = neighbors.get(neighbor).getOne().indexOf(name);
        boolean neighborWantsChat = neighbors.get(neighbor).getFour()
                .get(nameIndex);
     ...
     * @param name
     * @param neighbor
     * @return if both name and neighbor have expressed yes or no on whether to
     *         join the chat
     */
    ...
    public void updateChatWant(String name, boolean nameWantsChat,
            int neighborIndex) {
                ...
                new Thread() {
                    ...
                    public void run() {
                        try {
                            ...
                            System.out
                                    .println("handleConnection failed inside Server");
                            e.printStackTrace();
                        }
                    }
                }.start();
        ...
        try {
            for (String line = in.readLine(); line != null; line = in
                ...
                if (!line.equals("")) {
                    ...
                    // When the client first connects, it passes in the name of
                    // the boar
                    if (tokens[0].equals("name")) {
                        name = tokens[1];
                        if (!neighbors.contains(name)) {
                            // top, bottom, left, right
                            List<String> adjacents = Arrays.asList(null, null,
                                    null, null);
                            List<Boolean> invisibles = Arrays.asList(false,
                                    false, false, false);
                            List<Boolean> chatNeighbors = Arrays.asList(null,
                                    null, null, null);
                            Quadruple<List<String>, List<Boolean>, Socket, List<Boolean>> triple = new Quadruple<List<String>, List<Boolean>, Socket, List<Boolean>>(
                                    adjacents, invisibles, socket,
                                    chatNeighbors);
            ...
            // when the server thread specific to a client disconnects, revert
            // the neighboring boards to solid walls
    ...
    /**
     * When a client disconnects, find all boards joined to it, and revert to
     * solid walls also clear all the neighbors who are in chat
     * 
     * @param name
     */
    ...
    /**
     * do something with the client messages such as choosing the neighbor
     * clients to pass the ball to.
     * 
     * @param input
    ...
    private String handleRequest(String input) throws IOException {
        ...
        if (strangeTokens[0].equals("chatSend")) {
            ...
            int wallNum = neighbors.get(chatNeighbor).getOne()
                    .indexOf(chatName);
            String msg = strangeTokens[4];

            String delim = "123456789";
            String msgToReceive = "chatReceive" + delim + chatNeighbor + delim
                    + wallNum + delim + msg;
        ...
        if (tokens[0].equals("chatWant")) {
            ...
            boolean chatAgreed = checkChatAgreement(chatName, true,
                    chatNeighbor);

            if (chatAgreed && ended) {
                String msgToChatName = "chatCreated " + wallNum + " "
                        + chatNeighbor;
                String msgToChatNeighbor = "chatCreated "
                        + neighbors.get(chatNeighbor).getOne()
                                .indexOf(chatName) + " " + chatName;
        ...
        if (tokens[0].equals("create")) {
            ...
            try {
                ...
                Socket socketReceiver = neighbors.get(otherBoardName)
                        .getThree();

                outReceiver = new PrintWriter(socketReceiver.getOutputStream(),
                        true);

                String msgToReceiver = "create " + nameOfBall + " " + x + " "
                        + y + " " + xVel + " " + yVel + " " + radius;
        ...
        if (tokens[0].equals("port")) {
            ...
            try {
                ...
                if (neighbors.containsKey(otherBoardName)) {
                    ...
                    outReceiver = new PrintWriter(
                            socketReceiver.getOutputStream(), true);
                    String msgToReceiver = "port " + nameOfBoard + " "
                            + otherPortalName + " " + nameOfBall + " " + x
                            + " " + y + " " + xVel + " " + yVel + " " + radius;
                    outReceiver.println(msgToReceiver);
        ...
        if (tokens[0].equals("hit")) {
            ...
            boolean hitInvisible = neighbors.get(nameOfBoard).getTwo()
                    .get(wallNum);
            ...
            PrintWriter outReceiver;
            try {
                outSender = new PrintWriter(socketSender.getOutputStream(),
                        true);

                String neighbor = neighbors.get(nameOfBoard).getOne()
                        .get(wallNum);
